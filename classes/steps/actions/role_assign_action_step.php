...
class role_assign_action_step extends base_action_step {
    ...
    public function execute($step, $trigger, $event, $stepresults) {
        ...
        try {
            $result = role_assign($this->roleid, $datafields[$this->useridfield], $this->contextid);
        } catch (\Exception $exception) {
            $result = false;
        }

        $stepresults['assign_role_result'] = !empty($result);
        $stepresults['assign_role_result_id'] = $result;
